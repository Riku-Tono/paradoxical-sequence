...
PREV16_FACE_ORDER = [
    "INFLOW_FROM_32-63 | k=4 | pre3=2,2,3+",
    "START_IN_LAYER | k=2 | pre3=2",
    "NO_16_31_PASS",
]
LOW_SUPPORT_N = 10
...
def previous_face(events: list[dict[str, object]], from_bin: str, to_bin: str, no_pass_label: str) -> tuple[str, dict[str, object] | None]:
    layer_events = [e for e in events if e["from_bin"] == from_bin]
    pass_event = next((e for e in layer_events if e["to_bin"] == to_bin), None)
    return compact_event_face(pass_event, no_pass_label), pass_event
...
def main() -> None:
    ...
    for n in ids:
        ...
        common: dict[str, object] = {
            ...
            "first_8_15_index": first_entry["position"],
            "first_8_15_entry_route": first_entry["entry_route"],
            "first_8_15_transition": first_entry["transition"],
            "first_8_15_transition_k": first_entry["transition_k"],
            "first_8_15_pre_k_window_3": first_entry["pre_k_window_3"],
            "has_8_15_to_4_7": int(has_pass),
        }
    ...
    pass_rows_only = [r for r in first_pass_rows if r["has_8_15_to_4_7"] == 1]
        ...
        wait_rows.append(
            {
                ...
                "previous16_inflow_from_32_63_k4_pre223plus": prev16_counts[
                    "INFLOW_FROM_32-63 | k=4 | pre3=2,2,3+"
                ],
                "previous16_start_k2_pre2": prev16_counts["START_IN_LAYER | k=2 | pre3=2"],
    ...
    write_csv(OUT / "downstream8_step1_first_pass_table.csv", first_pass_rows)
    write_csv(OUT / "downstream8_step2_signature_counts.csv", signature_rows)
    write_csv(OUT / "downstream8_step3_wait_feature_table.csv", wait_rows)
    write_csv(
    ...
    write_csv(
        OUT / "downstream8_step5_link_from_32_63_face.csv",
        link_rows_from_counts(
            link32_counts,
            PREV32_FACE_ORDER,
            "previous_32_63_face",
    ...
    write_csv(
        OUT / "downstream8_step6_link_from_16_31_face.csv",
        link_rows_from_counts(
            link16_counts,
            PREV16_FACE_ORDER,
            "previous_16_31_face",
    ...
    write_csv(OUT / "downstream8_step7_postpass_windows.csv", postpass_rows)
    ...
    prev16_totals = Counter(str(r["previous_16_31_face"]) for r in pass_rows_only)
    ...
    report = [
        ...
        f"- Trajectories entering `8-15`: `{enter_count}`.",
        f"- Trajectories with a first `8-15 -> 4-7` pass: `{pass_count}`.",
        f"- Trajectories entering `8-15` without an observed `8-15 -> 4-7` pass: `{enter_count - pass_count}`.",
        "",
    ...
    for face in PREV16_FACE_ORDER:
        report.append(f"- `{face}`: `{prev16_totals[face]}`")
    report.append("")
    for face in PREV16_FACE_ORDER:
        total = sum(link16_counts[face].values())
        if total == 0:
            continue
        top_face, top_count = link16_counts[face].most_common(1)[0]
        flag = " LOW_SUPPORT_SOURCE" if total < LOW_SUPPORT_N else ""
        report.append(f"- Previous `{face}` top downstream face: `{top_face}` = `{top_count}/{total}`{flag}")
    ...
    (OUT / "downstream8_firstpass_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")
